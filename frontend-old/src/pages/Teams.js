...
const Teams = () => {
  const [teams, setTeams] = useState([
    {
      ...
      description: "Working on machine learning algorithms for academic projects",
      members: 4,
      maxMembers: 6,
      ...
      membersList: ["Abhishek Giri", "Deepali Chauhan", "Sidh Khurana", "Ayush Chauhan"]
    },
    {
      id: 2,
      ...
      creator: "Deepali Chauhan",
      membersList: ["Deepali Chauhan", "Harsh Rawat", "Muskan Sharma"]
    },
    {
      id: 3,
      ...
      creator: "Abhay Kanojia",
      membersList: ["Abhay Kanojia", "Ayush Chamoli"]
    }
  ]);
  ...
  const createTeam = () => {
    ...
    const team = {
      ...
      membersList: ['Current User']
    };
    setTeams([...teams, team]);
    setNewTeam({ name: '', description: '', maxMembers: 5, skills: [] });
    setShowCreateModal(false);
    alert(`Team created! Team code: ${code}`);
  };

  const joinTeamByCode = () => {
    const team = teams.find(t => t.code === joinCode.toUpperCase());
    if (team && team.members < team.maxMembers) {
      alert(`Joined ${team.name} successfully!`);
      setJoinCode('');
    } else {
      alert('Invalid team code or team is full!');
    }
  };
  ...
  const sendMessage = () => {
    if (chatMessage.trim()) {
      ...
      // Simulate real-time response from team member (for demo)
      setTimeout(() => {
        const responses = [
          'Got it!',
        ...
        const teamMembers = ['Deepali Chauhan', 'Sidh Khurana', 'Ayush Chauhan'];
          ...
          >
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                ...
                <span style={{ color: '#3b82f6', fontWeight: '700', fontSize: '1rem' }}>{team.members}/{team.maxMembers}</span>
              </div>
