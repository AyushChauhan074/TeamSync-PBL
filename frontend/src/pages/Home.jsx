                  ...
                  (searchQuery ? searchResults : [...users].reverse().slice(0, 5)).map(person => (
                    <div key={person.id} style={{ display: 'flex', alignItems: 'center', padding: '1rem', background: 'rgba(255,255,255,0.95)', borderRadius: '15px', color: '#333', backdropFilter: 'blur(10px)', border: '1px solid #e5e7eb' }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        <button onClick={() => sendMessage(person.id)} style={{ padding: '0.5rem 1rem', border: 'none', borderRadius: '8px', background: '#3b82f6', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.3rem', fontWeight: 'bold' }}>Chat</button>
                        
                        {person.github_username ? (
                          <a href={`https://github.com/${person.github_username}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.5rem', background: '#111827', color: 'white', borderRadius: '8px', textDecoration: 'none', transition: 'background 0.2s' }} title={`View ${person.name}'s GitHub Profile`}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.5rem', background: '#e5e7eb', color: '#9ca3af', borderRadius: '8px', cursor: 'not-allowed' }} title="No GitHub profile linked">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="none">
                              <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/>
                            </svg>
                          </div>
