...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        const data = await apiFetch(`/users/${parsedUser.userId}`);
        const apiUser = data.user || {};
        
        // Merge API user data with local fallback data
        const mergedUser = {
          ...parsedUser,
          ...apiUser,
          skills: apiUser.skills || enhancedUserData.skills,
          interests: apiUser.interests || enhancedUserData.interests,
          bio: apiUser.bio || enhancedUserData.bio,
          github_username: apiUser.github_username || enhancedUserData.github
        };
  ...
  const saveProfile = async () => {
    try {
      // Create payload matching the backend expectations
      const payload = {
      ...
      const data = await apiFetch(`/users/${user.id || user.userId}`, {
        method: 'PATCH',
        body: payload
      });
      ...
      // Update basic fields in local storage
