...
const Projects = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      try {
        const data = await apiFetch(`/projects/user/${parsedUser.userId}`);
  ...
  const getPhaseColor = (phase) => {
    const defaultPhase = phase || 'planning';
    switch (defaultPhase.toLowerCase()) {
      case 'planning': return { bg: '#fffbeb', text: '#d97706', border: '#fcd34d' };
      case 'development': return { bg: '#eff6ff', text: '#2563eb', border: '#bfdbfe' };
      case 'testing': return { bg: '#fef2f2', text: '#dc2626', border: '#fecaca' };
      case 'completed': return { bg: '#ecfdf5', text: '#059669', border: '#a7f3d0' };
      default: return { bg: '#f3f4f6', text: '#4b5563', border: '#e5e7eb' };
    }
  };

  if (loading || !user) return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', fontSize: '1.2rem', color: '#666' }}>Loading projects...</div>;
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
            ...
            <div style={{ padding: '2rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                <div style={{ padding: '1rem', background: '#f9fafb', borderRadius: '12px', border: '1px solid #e5e7eb' }}>
                  ...
                  <span style={{ color: '#111827', fontWeight: '600', fontSize: '0.95rem' }}>{project.team_name || `Team #${project.team_id}`}</span>
                </div>
                <div style={{ padding: '1rem', background: '#f9fafb', borderRadius: '12px', border: '1px solid #e5e7eb' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6b7280" strokeWidth="2">
                  ...
                  <span style={{ color: '#111827', fontWeight: '600', fontSize: '0.95rem' }}>{project.due_date ? new Date(project.due_date).toLocaleDateString() : 'Not set'}</span>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                ...
                <button 
                  onClick={() => project.github_repo_url && window.open(project.github_repo_url, '_blank')}
                  disabled={!project.github_repo_url}
                  style={{
                    background: '#f9fafb',
                    color: '#111827',
        ...
        {projects.length === 0 && (
          <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '3rem', background: 'white', borderRadius: '16px', color: '#6b7280' }}>
            No projects found. Create a team and start a project!
          </div>
        )}
      </div>
